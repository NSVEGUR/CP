def findMax(list1, n):
	return list1[n].index(max(list1[n]))
...
for i in range(rows):
	for j in range(cols):
		list1[i][j] = input(f'Enter element at row {i+1} & col {j+1} : ')
print("Peak of the given 2D array is: ", findPeak(list1, 0, rows - 1, rows))
